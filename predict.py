...
def run_prediction(prefecture="北海道", candidate=None):
    ...
    candidates["party_support"] = candidates["候補者届出政党の名称"].map(
        lambda x: party_support.get(x, 0.5)
    )
    ...
    y = (
        ...
        + 0.2 * candidates["has_dual"]
        + 0.1 * (1 - candidates["district_type"])
    ) > 5
    ...
    # 当選者を選出（確率順）
    result = candidates.sort_values("win_probability", ascending=False)
    ...
    # CSV出力
    result.to_csv("hokkaido_1to3_final_prediction.csv", index=False, encoding="utf-8-sig")
